...
class TripletMarginLoss(nn.Module):
    ...
    def forward(self, x1_encoded, x2_encoded, attentions1, attentions2, correspondences):
        ...
        n_valid = 0.0
        for idx in range(x1_encoded.shape[0]):
            l = self.triplet_margin_loss(x1_encoded, x2_encoded, attentions1, attentions2, correspondences, idx)
            if l is not None:
                loss += l
                n_valid += 1.0
        return loss / n_valid
...
class DistinctivenessLoss(nn.Module):
    ...
    def distinctiveness_loss(self, x1_encoded, x2_encoded, attentions1, attentions2, correspondences, idx):
        ...
        loss = torch.mean(torch.abs(scores1 - target1)) + torch.mean(torch.abs(scores2 - target2))# + ((torch.sum(attentions1) - torch.sum(scores1)) + (torch.sum(attentions2) - torch.sum(scores2))) / (1024 - ids.size(0))
