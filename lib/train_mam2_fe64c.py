...
if __name__ == "__main__":
    autoencoder = CorrespondenceEncoder.load_from_checkpoint("tb_logs/correspondence_encoder_lr1e3/version_0/checkpoints/epoch=44-step=6524.ckpt").requires_grad_(False)
    attentions = MultiAttentionLayer2(autoencoder)
    if REP_LOSS:
        tb_logger = TensorBoardLogger('tb_logs', name='attention_model_repeatability_loss')
    else:
        tb_logger = TensorBoardLogger('tb_logs', name='cfe64_multi_attention_model2_distinctiveness+_lossN32_lambda03_lowmargin')
